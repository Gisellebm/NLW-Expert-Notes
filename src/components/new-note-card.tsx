...
export function NewNoteCard({ onNoteCreated}: NewNoteCardProps) {
  ...
  function handleStartRecording() {
    ...
    if (!isSpeechRecognitionAPIAvailable) {
      toast.error('Infelizmente, seu navegador não suporta esta API de gravação.')
      return
    }
  ...
  function handleStopRecording() {
    ...
    return (
      <Dialog.Root>
        <Dialog.Trigger className='flex flex-col bg-slate-700 rounded-md p-5 gap-3 text-left outline-none hover:ring-2 hover:ring-slate-600 focus-visible:ring-2 focus-visible:ring-lime-400'>
        ...
        <Dialog.Portal>
          ...
          <Dialog.Content 
            className='fixed overflow-hidden inset-0 md:inset-auto md:left-1/2 md:top-1/2 md:-translate-x-1/2 md:-translate-y-1/2 md:max-w-[640px] w-full md:h-[60vh] bg-slate-700 outline-none md:rounded-md flex flex-col'
          >
